...
class Recovery extends CI_Controller
{
	...
	function __construct()
	{
		...
		$ver = $this->Recovery_model->get_version();
	...
	public function fun_login()
	{
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->Recovery_model->auntenticar($data);
	...
	public function enviarContrasena()
	{
		...
		if (($this->input->post('email', true))) {
			...
			$DATA_CORREO = $this->Recovery_model->get_password($email);
	...
	public function get_usuario()
	{
		if ($this->input->is_ajax_request()) {
			$email = $this->input->post('email');
			$data = $this->Recovery_model->get_user($email);
			echo json_encode($data);
		} else {
			show_404();
		}
	...
	public function update_tyc()
	{
		if ($this->input->is_ajax_request()) {
			...
			$this->Recovery_model->update_tyc($data, $id_usuario);
			echo json_encode($data);
		} else {
			show_404();
		}
	...
	{
		$nivel = $this->session->userdata('nivel');
		$consulta_controller = $this->Recovery_model->get_status_C();
