...
class FAQ extends CI_Controller
{
	...
	function __construct()
	{
		...
		$ver = $this->FAQ_model->get_version();
	...
	public function fun_login()
	{
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->FAQ_model->auntenticar($data);
	...
	public function get_usuario()
	{
		if ($this->input->is_ajax_request()) {
			...
			$data = $this->FAQ_model->get_user($email);
	...
	public function update_tyc()
	{
		if ($this->input->is_ajax_request()) {
			...
			$this->FAQ_model->update_tyc($data, $id_usuario);
			echo json_encode($data);
		} else {
			show_404();
		}
	...
	{
		$nivel = $this->session->userdata('nivel');
		$consulta_controller = $this->FAQ_model->get_status_C();
